...
def test_GetDispSign_FP(ctrlObj,TestingRange,AdminKeyInfo):
...
class DispSignCases():
    ...
    def positiveCase(self):
        ...
        certResult = self.CertRequest.genCert_with_RSA2048_Mix(False,True)
        if certResult[1]:
            CertDn=certResult[0]        
    ...
    def negativeCase_formatKey_voidParaChoice(self):
        ...
        noCertResult=self.CertRequest.clearCertAndKeyPairs(str_srcPin,str_srcPin) 
        if noCertResult:
            testResult=self.testCtrl.get_DispSign(None,None,None,None,str_srcPin)
            testResult=re.sub(re.compile('\s+'),' ',testResult)      
    ...
    def negativeCase_oneCert_voidParaChoice(self):
        ...
        certResult=self.CertRequest.genCert_with_RSA1024_Mix(False,True) 
        if certResult[1]: 
    ...
    def operationCase_PinUI_pinErr(self):
        ...
        certResult=self.CertRequest.genCert_with_RSA1024_Sign_P(False,True) 
        if certResult[1]:
            CertDn = certResult[0] 
    ...
    def positiveCase_supportedMixCertTest(self):
        ...
        for mix_cert in mixCertDict:
            certResult=self.CertRequest.genCert_With_Verify(mix_cert,str_srcPin,False,True)            
            if certResult[1]: 
                CertDn = certResult[0]                  
    ...
    def negativeCase_supportedCommCertTest(self):
        ...
        for comm_cert in commCertDict:
            certResult=self.CertRequest.genCert_With_Verify(comm_cert,str_srcPin,False,True)            
            if certResult[1]: 
                CertDn = certResult[0]                  
                for str_Hash in signHash:
    ...
    def positiveCase_supportedDispCertTest(self):
        ...
        for disp_cert in dispCertDict:
            certResult=self.CertRequest.genCert_With_Verify(disp_cert,str_srcPin,False,True)            
            if certResult[1]: 
                CertDn = certResult[0]                  
                for str_Hash in signHash:
    ...
    def negativeCase_rsaCert_errHashChoice(self):
        ...
        certResult=self.CertRequest.genCert_with_rsa1024M_and_rsa2048M(False,True)
    ...
    def negativeCase_sm2Cert_errHashChoice(self):
        ...
        logger.critical("beginning... || %s ",caseTitle)             
        certResult=self.CertRequest.genCert_with_SM2_Sign_P(False,True)  
        if certResult[1]: 
            CertDn = certResult[0]      
            for str_Hash in signHash:
...
if __name__ == '__main__':
    ...
    gcTest=DispSignCases(ctrlObj,AdminKeyList)  #建立类对象，打开控件测试页
    strPin = str_srcPin
    testResult= gcTest.positiveCase()   #测试用例集，根据测试范围定义测试强度
#      
#     testResult= gcTest.negativeCase_formatKey_voidParaChoice()
#      
#     testResult= gcTest.negativeCase_oneCert_voidParaChoice()
#       
#     testResult= gcTest.operationCase_PinUI_pressC()
#       
#     testResult= gcTest.operationCase_PinUI_pinErr()
#     # 
#     testResult= gcTest.operationCase_PinUI_pinLock()
#      
#     testResult= gcTest.operationCase_pressC()
#      
#     testResult= gcTest.operationCase_timeout()
#     #  
#     testResult= gcTest.operationCase_outKey()
#       
#     testResult= gcTest.negativeCase_rsaCert_errHashChoice()
#       
#     testResult= gcTest.negativeCase_sm2Cert_errHashChoice()
#       
#     testResult= gcTest.positiveCase_supportedDispCertTest()
#                
#     testResult= gcTest.positiveCase_supportedMixCertTest()
#        
#     testResult= gcTest.negativeCase_supportedCommCertTest()
#      
#     testResult= gcTest.negativeCase_noKey_voidParaChoice()
#      
#     testResult= gcTest.negativeCase_noKey_validParaChoice()
#      
#     testResult= gcTest.nagativeCase_manyKey_validParaChoice()
          
    gcTest.testCtrl.close()