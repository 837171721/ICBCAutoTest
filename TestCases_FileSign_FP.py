...
def test_GetFileSign_FP(ctrlObj,TestingRange,AdminKeyInfo):
...
class FileSignCases():
    ...
    def positiveCase(self):
        ...
        caseResult = None
        e=None
        certResult=self.CertRequest.genCert_with_RSA2048_Sign_P(False,True)  
        if certResult[1]: 
            CertDn=certResult[0]
    ...
    def negativeCase_noCert_voidParaChoice(self):        
        ...
        initResult = self.CertRequest.clearCertAndKeyPairs(str_srcPin,str_srcPin)
        if initResult[0]:        
...
if __name__ == '__main__':
    ...
    gcTest=FileSignCases(ctrlObj,AdminKeyList)  #建立类对象，打开控件测试页
    strPin = str_srcPin
